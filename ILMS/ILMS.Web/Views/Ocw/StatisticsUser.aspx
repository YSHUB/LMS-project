		...
		<div id="content">
			<ul class="nav nav-tabs mt-4" id="myTab" role="tablist">
				<li class="nav-item" role="presentation">
					<a class="nav-link active" id="UserTab" href="/OCW/StatisticsUser/0">콘텐츠 기준</a>
				</li>
				<li class="nav-item" role="presentation">
					<a class="nav-link" id="PartTab" href="/OCW/StatisticsPart/0">참여 기준</a>
				</li>
			</ul>
			<div class="card mt-4">
				<div class="card-body pb-1">
					<div class="form-row align-items-end">
						<div class="form-group col-6 col-md-2">
							...
							{
							%>
								<option value="<%:item.CodeValue%>" <%:item.CodeValue == Model.Gubun ? "selected" : ""%>><%:item.CodeName %></option>
							<%
							}
							%>
							</select>
						...
						<div class="form-group col-6 col-md-2">
							...
							<select id="ddlSort" name="Sort" class="form-control">
								<option value="COUNT" <%:Model.Sort == "COUNT" ? "selected" : "" %>>콘텐츠 등록순</option>
								<option value="USING" <%:Model.Sort == "USING" ? "selected" : "" %>>조회순</option>
								<option value="LIKE" <%:Model.Sort == "LIKE" ? "selected" : "" %>>추천순</option>
							</select>
						</div>
					...
					<div class="card">
						...
						<div class="card-body py-0">
							<div class="table-responsive">
								<table class="table table-hover table-horizontal" summary="콘텐츠통계-OCW기준">
									<caption>콘텐츠통계-OCW기준</caption>
									<thead>
										<tr>
											...
											<th scope="col" class="text-nowrap">콘텐츠 등록건수</th>
											<th scope="col" class="text-nowrap">누적 조회건수</th>
											<th scope="col" class="text-nowrap">누적 추천건수</th>
										</tr>
									</thead>
									<tbody>
									<% 
...
<asp:Content ID="Script" ContentPlaceHolderID="ScriptBlock" runat="server">
	<script type="text/javascript">
		...
		$("#btnSearch").click(function () {
			$("#mainForm").attr("action", "/Ocw/StatisticsUser/0").submit();
		});

		$("#btnExcelSave").click(function () {
			$("#mainForm").attr("action", "/Ocw/StatisticsExcel/0/U").submit();
		});
