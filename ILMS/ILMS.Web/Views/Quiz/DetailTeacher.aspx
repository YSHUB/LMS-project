	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$("#chgEnddtYes").click(function () {
				...
				ajaxHelper.CallAjaxPost("/Quiz/UpdateEndDay", { examno: <%:Model.ExamNo%>, endday: endDt }, "fnCompleteEndDay", "", "오류가 발생하였습니다. \n새로고침 후 다시 이용해주세요.");
			});

			// 출제완료
			$("#completeYes").click(function () {
				if (!chk) {
					ajaxHelper.CallAjaxPost("/Quiz/QuizComplete", $("#mainForm").serialize(), "fnCompleteQuiz", "'Y'", "오류가 발생하였습니다. \n새로고침 후 다시 이용해주세요.");
					chk = true;
				} else {
			...
			$("#IncompleteYes").click(function () {
				ajaxHelper.CallAjaxPost("/Quiz/QuizUnComplete", { examno: <%:Model.ExamNo%> }, "fnCompleteQuiz", "'N'", "오류가 발생하였습니다. \n새로고침 후 다시 이용해주세요.");
			});
		...
		function fnChkExaminee(gbn) {
			ajaxHelper.CallAjaxPost("/Quiz/ChkExaminee", { courseno: <%:Model.CourseNo%>, examno: <%:Model.ExamNo%> }, "fnChkExamineeResult", "'" + gbn + "'", "오류가 발생하였습니다. \n새로고침 후 다시 이용해주세요.");
		}
		...
		function fnChkExamineeResult(gbn) {
			...
			if (result > 0) {
				...
				if (gbn == "D") fnDelete();
				else location.href = "/Quiz/Write/<%:Model.ExamDetail.CourseNo %>/<%:Model.ExamDetail.ExamNo %>";
			}
		}
		...
		function fnDelete() {
			ajaxHelper.CallAjaxPost("/Quiz/QuizDelete", $("#mainForm").serialize(), "fnDeleteQuiz", "'N'", "오류가 발생하였습니다. \n새로고침 후 다시 이용해주세요.");
		}

		function fnDeleteQuiz() {
			var result = ajaxHelper.CallAjaxResult();
			
			if (result > 0) {
				bootAlert("삭제되었습니다.", function () {
					location.href = "/Quiz/ListTeacher/<%:Model.ExamDetail.CourseNo %>";
				});
			} else {
				bootAlert("실행 중 오류가 발생하였습니다.");
			}
...
<asp:Content ID="Content" ContentPlaceHolderID="ContentBlock" runat="server">
	<%
		...
		string EstimationGubunSt = Model.ExamDetail.EstimationGubun;    // 퀴즈상태
	...
	<div class="card card-style01">
		...
		<div class="card-body collapse d-md-block" id="collapseExample2">
			<ul class="list-inline list-inline-style01">
				...
				<li class="list-inline-item"><%:Model.ExamDetail.IsGradingNm %></li>
				<li class="list-inline-item"><%:Model.ExamDetail.OpenYesNoNm %></li>
				<li class="list-inline-item"><%:Model.ExamDetail.SETypeNm %></li>
				<%
				if (Model.ExamDetail.UseMixYesNo.Equals("Y"))
				{
				%>
				...
				if (Model.ExamDetail.ExampleMixYesNo.Equals("Y"))
				{
				%>
			...
			<div class="row mt-2 align-items-end">
				<div class="col-md">
					...
					<dl class="row dl-style02">
						...
						<dd class="col"><%:(Model.ExamDetail.SEType.Equals(0) && Model.ExamDetail.SE0StateGbn.Equals(-1)) ? "응시대기" : Model.ExamDetail.StartDayFormat + ((Model.ExamDetail.SEType.Equals(0) && Model.ExamDetail.SE0State.Equals(1)) ? "" : " " + Model.ExamDetail.StartHours + ":" + Model.ExamDetail.StartMin) + " ~ " + ((Model.ExamDetail.SEType.Equals(0) && Model.ExamDetail.SE0State.Equals(1)) ? "임의종료시점까지" : Model.ExamDetail.EndDayFormat + " " + Model.ExamDetail.EndHours + ":" + Model.ExamDetail.EndMin) %></dd>
					</dl>
				</div>
	...
	<div class="row align-items-center mt-2">
		<div class="col-md">
			<p class="mb-0 font-size-14 text-danger font-weight-bold <%:(TakeTypeYn) ? "" : "d-none" %>"><i class="bi bi-info-circle-fill"></i> 퀴즈 미리보기는 출제한 퀴즈 확인용이며, 저장 후에는 수정이 불가능합니다.</p>
		</div>
		<div class="col-md-auto mt-2 mt-md-0 text-right">
			...
			<button type="button" class="btn btn-secondary <%:(TakeTypeYn) ? "" : "d-none" %>" onclick="fnOpenPopup('/Quiz/Run/<%:Model.CourseNo %>/<%:Model.ExamNo %>', 'PreViewQuiz', 1500, 830, 0, 0, 'auto')" title="새창열림">미리보기</button>
		</div>
	</div>	
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-body">
					<p>출제완료를 하시면 더 이상 문제를 수정하실 수 없으며, <%:ConfigurationManager.AppSettings["StudentText"].ToString() %>들이 시험에 응시할 수 있도록 공개로 설정됩니다. </p>
					<p>지금 출제완료를 하시겠습니까?</p>
				</div>
