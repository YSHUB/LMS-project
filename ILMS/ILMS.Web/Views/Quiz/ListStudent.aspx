...
<asp:Content ID="Content" ContentPlaceHolderID="ContentBlock" runat="server">
	...
	<div class="card card-style01">
		<div class="card-header">
			...
			<a href="/Quiz/DetailStudent/<%:item.CourseNo %>/<%:item.ExamNo %>" class="card-title01 text-dark"><%:item.ExamTitle %></a>
		</div>
			...
			<div class="row mt-2 align-items-end">
				...
				<div class="col-md-4 mt-2 mt-md-0 text-right">
					<div class="<%:(ConfigurationManager.AppSettings["ExamGradeOpenYN"].Equals("Y") && item.EstimationGubun.Equals("EXET005")) ? "btn-group btn-group-lg" : "" %>">
						<a class="btn btn-lg btn-point" href="/Quiz/DetailStudent/<%:item.CourseNo %>/<%:item.ExamNo %>">자세히</a>
						<a class="btn btn-lg btn-dark <%:(ConfigurationManager.AppSettings["ExamGradeOpenYN"].Equals("Y") && item.EstimationGubun.Equals("EXET005")) ? "" : "d-none" %>" href="/Quiz/EstimationStudent/<%:item.CourseNo %>/<%:item.ExamNo %>/<%:item.ExamineeNo %>">결과보기</a>
					</div>
				</div>
			</div>
		</div>
