	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			$("#btnExcel").click(function () {
				location.href = "/Quiz/AdminExcel/" + $("#ddlProgram").val() + "/" + $("#ddlTermYear").val() + (($("#txtSearch").val().trim() == "") ? "" : "/" + $("#txtSearch").val());
			});
		})
	</script>
</asp:Content>
...
<asp:Content ID="Content" ContentPlaceHolderID="ContentBlock" runat="server">
	<form action="/Quiz/ListAdmin/<%:Model.ProgramNo %>" id="mainForm" method="post">
		<div id="content">
			<div class="card mt-4">
				...
				<div class="col-12 mt-2">
					...
					<div class="card">
						...
						<div class="card-body py-0">
							<div class="table-responsive">
								<table class="table table-hover table-sm table-horizontal" summary="퀴즈관리 리스트">
									<caption>퀴즈관리 리스트</caption>
									<thead>
										<tr>
											<%
											...
											{
											%>
											<th scope="col" class="text-nowrap d-md-table-cell">과정</th>
											<%
											}
											%>
											...
											<th scope="col" class="text-nowrap">등록퀴즈수</th>
					...
					%>		
										<tr>
											...
											{
											%>
											<td class="text-center d-md-table-cell"><%:item.ProgramName %></td>
											<%
											}
											%>
											...
											<td><%:item.CreateCnt %></td>
											<td class="text-nowrap"><a class="font-size-20 text-primary" href="/Quiz/DetailAdmin/<%:item.CourseNo %>?ProgramNo=<%:item.ProgramNo%>&TermNo=<%:Model.TermNo %>&SearchText=<%:Model.SearchText %>&PageRowSize=<%:Model.PageRowSize%>&PageNum=<%:Model.PageNum%>" title="상세보기"><i class="bi bi-card-list"></i></a></td>
										</tr>
					<%
							}
					%>
