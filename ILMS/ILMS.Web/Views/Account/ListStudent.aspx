...
<asp:Content ID="Content" ContentPlaceHolderID="ContentBlock" runat="server">
    ...
    <form action="/Account/ListStudent" method="post" id="mainForm">

        <ul class="nav nav-tabs">
            <li class="nav-item"><a class="nav-link active show" href="ListStudent"><%:ConfigurationManager.AppSettings["StudentText"].ToString() %></a> </li>
                ...
                <table class="table" summary="<%:ConfigurationManager.AppSettings["EmpIDText"].ToString() %> 리스트">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><%:item.Email%></td>
                            <td>
                                <a href="<%: ConfigurationManager.AppSettings["BaseUrl"] %>/Account/AutoLogOnProcess/?userNo=<%:item.UserNo %>&returnUrl=<%:ConfigurationManager.AppSettings["BaseUrl"]  %>/Mypage/LectureRoom" class="btn btn-sm btn-outline-dark">입장</a>
                            </td>
...
<asp:Content ID="Script" ContentPlaceHolderID="ScriptBlock" runat="server">
    <script>
        ...
        function fnTest(userID) {
            console.log(userID);
            ajaxHelper.CallAjaxPost("/Account/AutoLogonProcess", { "paramUserID": userID }, "fnCallBackByTest");
		}

        function fnCallBackByTest() {
            var result = ajaxHelper.CallAjaxResult();
            console.log(result);
            window.open(result,'user')
        }

        function fnGo(userNo) {

			window.location = "/Account/DetailStudent/" + userNo + "?AssignNo=" + '<%:Model.AssignNo %>' + "&UserType=" + '<%:Model.UserType%>' + "&SearchOption=" + '<%:Model.SearchOption%>' + "&SearchText=" + encodeURI(encodeURIComponent('<%:Model.SearchText%>')) + "&PageRowSize=" + <%:Model.PageRowSize%> + "&PageNum=" + <%:Model.PageNum%>;
		}
	</script>
</asp:Content>
