...
namespace ILMS.Web.Controllers
{
	...
	[RoutePrefix("Exam")]
	public class ExamController : LectureRoomBaseController
	{
		...
		public ActionResult Write(int? param1, int? param2)
		{
			...
			vm.AddExamGbn = (Request.QueryString["AddExamYesNo"] != null) ? (!Request.QueryString["AddExamYesNo"].Equals("Y") ? "N" : Request.QueryString["AddExamYesNo"].ToString()) : "N";
			vm.AddExamRowstate = (vm.AddExamGbn.Equals("Y")) ? "C" : "U";
			
			// 공통코드
			Code code = new Code("A", new string[] { "CINM" });
			code.DeleteYesNo = "N";
			code.UseYesNo = "Y";
			vm.baseCodes = baseSvc.GetList<Code>("common.DETAILCODE_SELECT_A", code).Where(c => c.CodeValue != "CINM001").ToList();
			...
			if (!vm.ExamNo.Equals(0))
			{
				...
				vm.ExamDetail.AddExamYesNo = (vm.AddExamGbn.Equals("Y")) ? vm.AddExamGbn : vm.ExamDetail.AddExamYesNo;
				...
				if (vm.ExamDetail.AddExamYesNo.Equals("Y"))
				{
					...
					vm.ExamineeList = baseSvc.GetList<Examinee>("exam.EXAMINEE_SELECT_B", examinee).ToList();
				}
				else
				{
			...
			else
			{
				...
				vm.ExamDetail.AddExamYesNo = "N";
				vm.AddExamRowstate = null;
		...
		public ActionResult ListStudent(int param1, int? param2)
		{
			ExamViewModel vm = new ExamViewModel();
		...
		public ActionResult ExamineeExcel(int param1, int param2, string hdnExamStatus, string hdnSearchGubun, string hdnSearchText, string hdnSearchSortGubun, string hdnadminYn)
		{
			...
			if (hdnadminYn.Equals("Y"))
			{
				...
				{
					headers = new string[] { "No", "소속", ConfigurationManager.AppSettings["StudIDText"].ToString(), "이름", "IP", "상태", "응시일시", "제출일시", "경과시간", "총점" };
					colums = new string[] { "ExamineeRowNum", "AssignName", "UserID", "HangulName", "ExamUserIpAddr", "ExamStatusNm", "TakeDateTimeFormatView", "LastDateTimeFormatView", "RemainTimeFormat", "ExamTotalScore" };
				}
		...
		public ActionResult DetailAdminExcel(int param1, string param2)
		{
			...
			headers = new string[] { "주차", "차시", "시험제목", "응시방법", "응시기간", "상태", "응시인원", "평가인원" };
			columns = new string[] { "Week", "InningSeqNo", "ExamTitle", "LectureTypeNm", "ExamDate", "EstimationGubunNm", "TakeStudentCount", "EstimationCount" };
			excelFileName = String.Format("강의운영관리_시험관리_상세보기_{0}", DateTime.Now.ToString("yyyyMMdd"));
		...
		public ActionResult AdminExcel(int param1, int param2, string param3)
		{
			...
			{
				headers = new string[] { "과정", "강의형태", WebConfigurationManager.AppSettings["SubjectText"].ToString(), "분반", "담당" + WebConfigurationManager.AppSettings["ProfIDText"].ToString(), "등록시험수" };
				colums = new string[] { "ProgramName", "StudyTypeName", "SubjectName", "ClassNo", "ProfessorNm", "CreateCnt" };
			}
			else
			{
				headers = new string[] { "강의형태", WebConfigurationManager.AppSettings["SubjectText"].ToString(), "분반", "담당" + WebConfigurationManager.AppSettings["ProfIDText"].ToString(), "등록시험수" };
				colums = new string[] { "StudyTypeName", "SubjectName", "ClassNo", "ProfessorNm", "CreateCnt" };
			}

			return ExportExcel(headers, colums, vm.QuizList, String.Format("시험목록{0}", DateTime.Now.ToString("yyyyMMdd")));
		}

		public JsonResult WeekList(int courseno, int examno, string lessonform)
		{
			// 주차
			Inning inning = new Inning("C");
			inning.CourseNo = courseno;
			inning.ExamNo = examno;
			inning.LessonForm = lessonform;

			List<Inning> WeekList = baseSvc.GetList<Inning>("course.COURSE_INNING_SELECT_C", inning).ToList();

			return Json(WeekList);
		}

		public JsonResult InningList(int courseno, int examno, string lessonform, int weekno)
		{
			// 차시
			Inning inning = new Inning("D");
			inning.CourseNo = courseno;
			inning.ExamNo = examno;
			inning.LessonForm = lessonform;
			inning.Week = weekno;

			List<Inning> InningList = baseSvc.GetList<Inning>("course.COURSE_INNING_SELECT_D", inning).ToList();

			return Json(InningList);
		}

		public JsonResult InningInfo(int courseno, int inningno)
		{
			// 차시정보
			Inning inning = new Inning("E");
			inning.CourseNo = courseno;
			inning.InningNo = inningno;

			Inning InningInfo = baseSvc.Get<Inning>("course.COURSE_INNING_SELECT_E", inning);

			return Json(InningInfo);
		}
		...
		public JsonResult ExamCreate(ExamViewModel vm)
		{
			int result = 0;

			if (vm.ExamDetail.ExamNo == 0 || vm.ExamDetail.AddExamRowstate.Equals("C")) vm.ExamDetail.RowState = "C";
		...
		public JsonResult ExamDelete(ExamViewModel vm)
		{
			int result = 0;
		...
		public JsonResult ExamComplete(ExamViewModel vm)
		{
			int result = 0;
		...
		public JsonResult ExamUnComplete(int examNo)
		{
			int result = 0;
		...
		public JsonResult UpdateExamReset(int courseNo, int examNo, int examineeNo, long userNo)
		{
			int result = 0;
		...
		public JsonResult OffCreate(ExamViewModel vm)
		{
			...
			if (Request.Files.Count > 0)
			{
				...
				{
					fileGroupNo = FileUpload(vm.ExamineeDetail.OFFFile.Equals("") ? "C" : "U", "ExamOff", vm.ExamineeDetail.OFFFile, "OffFile");
					if (fileGroupNo <= 0)
						fileSuccess = false;
				}
			}
